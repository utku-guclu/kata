function largestProductinaSeries(n) {
  ...
  return adjMax1d(thousandDigits, 0, -Infinity, n);
}

function adjMax1d(arr, currIdx, max, adjDigits) {
  // base case: stop when we reach the second last element
  if (currIdx > arr.length - adjDigits) return max;

  let prod = 1;

  for (let nextIdx = 1; nextIdx <= adjDigits; nextIdx++) {
    prod *= arr[currIdx + nextIdx];
  }
  ...
  return adjMax1d(arr, currIdx + 1, max, adjDigits);
}

const res = largestProductinaSeries(13);
console.log(res);
